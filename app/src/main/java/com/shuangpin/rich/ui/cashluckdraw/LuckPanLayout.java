...
public class LuckPanLayout extends RelativeLayout {
    ...
    public LuckPanLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        CashLogger.setDebug(true);
//        backgroundPaint.setColor(Color.rgb(255,92,93));
        backgroundPaint.setColor(Color.rgb(255,172,0));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        MinValue -= CashUtil.dip2px(context,10)*2;
        CashLogger.getLogger().d("screenWidth = "+screenWidth + "screenHeight = "+screeHeight + "MinValue = "+MinValue);
    ...
    private void drawSmallCircle(boolean FirstYellow){
        int pointDistance = radius - CashUtil.dip2px(context,10);
        for(int i=0;i<=360;i+=20){
            int x = (int) (pointDistance * Math.sin(CashUtil.change(i))) + CircleX;
            int y = (int) (pointDistance * Math.cos(CashUtil.change(i))) + CircleY;

            if(FirstYellow)
                canvas.drawCircle(x,y, CashUtil.dip2px(context,4),yellowPaint);
            else
                canvas.drawCircle(x,y, CashUtil.dip2px(context,4),whitePaint);
